...
const Allpost = () => {
  ...
  const [accidentPostData, setaccidentPostData] = useState([]);
  ...
  useEffect(() => {
    ...
    SanityClient.fetch(
      ...
      .then((data) => setaccidentPostData(data))
      .catch((error) => console.error(error));
  }, []);
              ...
              <Link to={`/post/${post.slug.current}`} className="block">
                {post.image?.asset?.url && (
                  <img
                    ...
                    className="w-full h-60 object-fill mb-4 hover:scale-105 transition-transform duration-300"
                  />
                )}
...
<div className="w-full bg-gray-900">
      <main className="mx-auto min-h-screen max-w-8xl py-10 px-10">
        ...
        <h1
          className="text-xl font-bold mb-8 text-white hover:text-blue-300"
          style={{ textShadow: "0px 0px 10px blue", cursor:'pointer' }}
          onClick={() => window.open('/accidentnews')}
          >
         View All ➡️
        </h1>
          </div>
